    ...
    public function __invoke(GetPlayerDevelopmentDashboardRequest $request): JsonResponse
    {
        try {
            ...
            $data = Cache::remember($cacheKey, 120, function () use ($teamScopeId, $playerId, $days, $player) {
                ...
                $assessmentPrevQuery = clone $assessmentBaseQuery;
                if ($assessmentLatest?->id) {
                    $assessmentPrevQuery->where('id', '!=', $assessmentLatest->id);
                }
                $assessmentPrev = $assessmentPrevQuery
                    ->orderByDesc('assessment_date')
                    ->orderByDesc('created_at')
                    ->first();
                ...
                return [
                    ...
                    'baseline' => [
                        ...
                        'mobility_score' => $mobilityInitial,
