...
class GetNextWeekPlanDraft extends Controller
{
    public function __invoke(string $teamId, Request $request, NextWeekPlanGeneratorService $service): JsonResponse
    {
        if (! $this->canAccessTeam($request, $teamId)) {
            return response()->json([
                'code' => 'NWD-F',
                'message' => 'not allowed to generate next week plan draft for this team',
                'status' => 'error',
                'data' => [],
        ...
        return response()->json([
            'code' => 'NWD',
            'message' => 'next week plan draft',
            'status' => 'success',
            'data' => $service->generateForTeam($teamId, $this->options($request)),
        ], HttpCodes::HTTP_OK);
    }
