    ...
    private function recommendationFromLimiter(array $limiter, array $trends, array $dna): array
    {
        ...
        return match ($id) {
            ...
            'weighted-ball-to-mound-transfer' => [
                'id' => 'transfer-weighted-ball-velo-to-mound',
                'category' => 'throwing',
                'priority' => 'high',
                'title' => 'Transfer Weighted-Ball Velocity to the Mound',
                'why' => 'The 5 oz weighted-ball velocity is ahead of current mound velocity.',
                'action' => 'Pair 5 oz intent throws with short mound velocity sets and track whether the gap closes.',
                'evidence' => $limiter['evidence'] ?? [],
                'expected_gain' => '+1-3 mph mound velocity',
                'confidence' => $limiter['confidence'] ?? 'medium',
            ],
            'five-oz-transfer-flat' => [
                'id' => 'improve-five-oz-transfer',
                'category' => 'throwing',
                'priority' => 'medium',
                'title' => 'Improve 5 oz Velocity Transfer',
                'why' => 'The current 5 oz velocity is not above the assessment pitch-velocity baseline.',
                'action' => 'Use a consistent 5 oz pulldown or mound baseline test before changing the weighted-ball block.',
                'evidence' => $limiter['evidence'] ?? [],
                'expected_gain' => '+1-2 mph 5 oz velocity',
                'confidence' => $limiter['confidence'] ?? 'medium',
            ],
            'underload-speed-transfer' => [
                'id' => 'convert-underload-speed',
                'category' => 'throwing',
                'priority' => 'medium',
                'title' => 'Convert Underload Speed',
                'why' => 'The 3 oz ball is much faster than the 5 oz ball, so arm speed is not fully carrying into the game ball.',
                'action' => 'Use contrast throws: 3 oz speed, 5 oz intent, then mound fastballs with the same move.',
                'evidence' => $limiter['evidence'] ?? [],
                'expected_gain' => '+1-2 mph game-ball velocity',
                'confidence' => $limiter['confidence'] ?? 'medium',
            ],
            'overload-strength' => [
                'id' => 'build-overload-strength',
                'category' => 'throwing',
                'priority' => 'medium',
                'title' => 'Build Overload Strength',
                'why' => 'Velocity drops sharply with the 7 oz ball, showing overload strength or sequencing limitations.',
                'action' => 'Add low-volume 6/7 oz strength throws and lower-half sequencing work before max intent.',
                'evidence' => $limiter['evidence'] ?? [],
                'expected_gain' => 'Flatter weighted-ball velocity spectrum',
                'confidence' => $limiter['confidence'] ?? 'medium',
            ],
            'velocity-spectrum-efficiency' => [
                'id' => 'tighten-velocity-spectrum',
                'category' => 'throwing',
                'priority' => 'medium',
                'title' => 'Tighten Velocity Spectrum',
                'why' => 'The 5 oz to 3 oz velocity ratio is below target.',
                'action' => 'Repeat the weighted-ball set with consistent intent and compare 3 oz, 5 oz, and 7 oz max velocities.',
                'evidence' => $limiter['evidence'] ?? [],
                'expected_gain' => 'Improved 5 oz / 3 oz velocity ratio',
                'confidence' => $limiter['confidence'] ?? 'medium',
            ],
            default => [
