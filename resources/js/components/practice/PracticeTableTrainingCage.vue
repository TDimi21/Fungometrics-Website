...
const { axiosDelete } = useAxiosAuth()
...
const sessionTitle = () => 'Cage Practice'
...
const swingCount = (item) => {
  const count = Number(
    (Array.isArray(item?.cage) ? item.cage.length : null) ??
    item?.result_count ??
    item?.balls ??
    item?.total_balls ??
    item?.count ??
...
const resumePractice = async (players) => {
  ...
  console.log(cageHeight.value.ft);
...
<template>
  <section class="practice-list-container mt-6">
    <div v-if="isLoading" class="list-state">Loading data...</div>
    <div v-else-if="!tableData.length > 0" class="list-state">No found data</div>
      ...
      >
        ...
        <div class="practice-card-right">
          ...
          <div class="card-actions">
            ...
            <button
              @click="router.push({ name: 'training.statsCage', params: { 'idPractice': item.id, 'mode': item.mode, 'isComplete': item.is_completed } })"
              class="card-action-btn"
              title="Stats"
            >
              <TableStats />
