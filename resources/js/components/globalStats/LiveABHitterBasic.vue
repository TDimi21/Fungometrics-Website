...
<template>
  <div class="batting-table-wrap">
    <h1 class="batting-table-title">Live AB - Hitter Basic</h1>
      ...
      <table class="batting-stat-table">
        ...
        <tbody>
          <tr v-if="props.team" class="team-row">
            <td>TEAM TOTAL</td>
            <td v-for="col in columns.slice(1)" :key="`liveab-basic-team-${col.key}`">
          ...
          <tr
            ...
            :key="`liveab-basic-row-${idx}`"
            class="data-row"
          >
            <td>{{ item.player ?? '?' }}</td>
            <td v-for="col in columns.slice(1)" :key="`liveab-basic-${idx}-${col.key}`">
              {{ item[col.key] ?? 0 }}
            </td>
          </tr>
        </tbody>
