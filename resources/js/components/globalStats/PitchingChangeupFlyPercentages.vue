  ...
  <div class="grid grid-cols-1 md:grid-cols-2">
    <div>
      <h1 class="text-fungo-red text-2xl text-center mt-9 mb-6 font-fungo-700">
        Pitching Session - ChangeUp - Percentages
      </h1>
    ...
    <div>
      <h1 class="text-fungo-red text-2xl text-center mt-9 mb-6 font-fungo-700">
        Pitching Session - Fly Balls - Percentages
      </h1>
      <section class="px-[10%] md:px-[5%] mt-4 overflow-x-auto">
        <table class="w-full border-separate space-y-6 text-fungo-darkblue">
          <thead class="bg-fungo-lightblue">
          ...
          <tbody>
            <tr v-if="props.teamFB != null">
              <td class="text-center">
                Total Players
                <!-- <img :src="item.player.avatar" alt="" class="w-16 h-full object-center object-cover mx-auto rounded-full"/> -->
              </td>
              <td class="text-center">{{ props.teamFB.pitches ?? '?'}}</td>
              <td class="text-center">{{ props.teamFB.FB ?? '?' }}</td>
              <td class="text-center">{{ props.teamFB.CH ?? '?' }}</td>
              <td class="text-center">{{ props.teamFB.CB ?? '?' }}</td>
              <td class="text-center">{{ props.teamFB.SL ?? '?' }}</td>
            </tr>
            <tr v-if="props.playersFB == null">
              <td colspan="16" class="text-fungo-darkblue text-3xl text-center">No found data</td>
            </tr>
            <tr v-else v-for="(item, index) in props.playersFB">
              <td class="text-center">
                {{ item.player ?? '?' }}
                <!-- <img :src="item.player.avatar" alt="" class="w-16 h-full object-center object-cover mx-auto rounded-full"/> -->
              </td>
