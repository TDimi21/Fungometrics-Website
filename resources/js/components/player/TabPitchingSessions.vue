...
const tableHeadings = ref([
  "PITCHERS", "TOTALS BALLS", "STATUS/CREATED AT", "START/RESUME", "STATS"
])

const { resumenBullpenPlayer } = usePlayerResume()

const getTrainigs = async(page = 1) => {
  try {
    isLoading.value = true
    await axiosGet(`player/sessions/bullpen`).then((response) => {
      if (response) {
        let array = response.data.data.data
        tableData.value = array.filter(item => item.type == 'P')
