...
const filters = [
  ['all', 'All'], ['needs_review', 'Needs Review'], ['matched', 'Matched'],
  ['skipped', 'Skipped'], ['pitcher', 'Pitchers'], ['batter', 'Batters'],
]
...
const summary = computed(() => ({
  ...
  matched: props.players.filter(player => status(player) === 'matched').length,
  review: props.players.filter(player => status(player) === 'needs_review').length,
  skipped: props.players.filter(player => status(player) === 'skipped').length,
}))
const duplicateGroups = computed(() => {
  const groups = {}
  props.players.forEach(player => {
    const target = mappedValue(player)
    if (target && target !== '__skip__') (groups[target] ||= []).push(player)
  })
...
<template>
  <section class="player-mapping">
    <header class="summary">
      ...
      <div class="counts"><b>{{ summary.found }}<small>Players found</small></b><b>{{ summary.matched }}<small>Matched</small></b><b>{{ summary.review }}<small>Need review</small></b><b>{{ summary.skipped }}<small>Skipped</small></b></div>
    </header>
