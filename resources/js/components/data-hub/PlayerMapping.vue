...
<template>
  <section class="player-mapping">
    <header class="summary">
      ...
      <div class="counts"><b>{{ summary.found }}<small>Players found</small></b><b>{{ summary.connected }}<small>Connected</small></b><b>{{ summary.notImporting }}<small>Not Importing</small></b></div>
    </header>
    ...
    <div class="cards">
      <article v-for="player in visible" :key="player.source_key" :class="['player-card',status(player)]">
        ...
        <label class="selector"><span>FMTRX roster player</span><select :value="mappedValue(player)" @change="emit('update:mapping',player.source_key,$event.target.value)"><option value="">— Not Importing —</option><option v-for="candidate in teamPlayers" :key="candidate.id" :value="candidate.id">{{ rosterLabel(candidate) }}</option></select><small>{{ status(player).replace('_',' ') }}<template v-if="duplicateFor(player)"> · duplicate target</template></small><div><button v-if="mappedValue(player)" @click="emit('update:mapping',player.source_key,'')">Set to Not Importing</button></div></label>
      </article>
