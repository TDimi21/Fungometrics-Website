...
const canImportData = computed(() =>
  accessStore.canAccess('data_hub_import')
  || Boolean(accessStore.summary?.capabilities?.subscription_admin)
);
...
const confirmLogout = () => {
  ...
  endWebSession(api_url).catch(() => {}).finally(() => {
    window.dispatchEvent(new Event('fmtrx-logout'));
    accessStore.clear();
        ...
        <div class="top-brand-content">
          ...
          <div class="top-brand-social">
            ...
            <div class="flex items-center gap-1 ml-3 pl-3 border-l border-white/20">
              <RouterLink v-if="canImportData" to="/data-hub" class="flex items-center gap-1 text-white/80 hover:text-white text-xs font-bold uppercase tracking-wide px-3 py-1.5 rounded-lg bg-red-500/15 hover:bg-red-500/25 border border-red-400/25 hover:border-red-400/45 transition" title="Import Data">
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 16V4m0 0L7 9m5-5 5 5M5 14v4a2 2 0 002 2h10a2 2 0 002-2v-4"/></svg>
                Import Data
              </RouterLink>
