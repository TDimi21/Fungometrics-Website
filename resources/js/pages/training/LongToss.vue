...
const throwsStore = ref(training.trainingActive.countThrow ?? 0);
const throws = ref(0);
...
const save = async () => {
  ...
  //   distance: dataProcess.value.distance,
  //   hop: dataProcess.value.hop,
  //   sort: sort.value,
  //   set: set.value,
  // }
  let dataToSave = {
  ...
  try {
    if(dataProcess.value.hop != null && dataProcess.value.hop >= 0){
      if(training.trainingActive.set != null){
        await axiosPut('result/longtoss/' + training.trainingActive.id, {
      ...
      await axiosPost('result/longtoss', dataToSave).then(async (response) => {
      if (response) {
        isLoading.status = !isLoading.status;
...
const goToStadistics = () => {
  let link = router.resolve({ name: 'training.statsMode', params: { 'idPractice': dataProcess.value.practice, 'mode': "LT" } })
...
const changeHop = (value)=> {
  if(dataProcess.value.hop == value){
    dataProcess.value.hop = 0
  }else{
    dataProcess.value.hop = value
  }
}
</script>
<template>
        ...
        <div class="flex flex-col md:flex-row gap-4 lg:gap-12 justify-evenly lg:justify-start h-auto md:h-[6em] bg-white rounded rounded-lg">
          <div class="flex">
            ...
            <div class="lg:ml-5">
              ...
              <h1 class="text-fungo-darkblue text-[1.2em] font-fungo-800 lg:text-[1.7em] lg:-mt-2">Long Toss</h1>
            </div>
          </div>
          <div
    ...
    <div class="grid grid-flow-row md:grid-cols-2 xl:grid-cols-3 justify-center mt-2 xl:mt-5 gap-3">
      <div class="mx-10 px-10">
        <div class="flex justify-center">
          <div class="mt-5 order-1 xl:col-span-5">
            ...
            <div v-if="training.trainingActive.set == null" class="text-[16px] xl:text-[24px] text-fungo-blue2 mb-4">
              <button @click="openModal">end long toss practice ></button>
            </div>
            <div v-if="training.trainingActive.set == null">
              ...
              <div class="flex w-full justify-between bg-white p-4 mb-2">
                ...
                <!-- <span>{{ training.countThrowForSet }}</span> -->
      ...
      <div class="bg-white p-3 ">
        <div class="grid grid-rows-1 grid-flow-col justify-start">
          <LongTossIcon></LongTossIcon>
          <h2 class="text-[30px] font-fungo-800 text-fungo-red place-self-center">Long Toss</h2>
        </div>
        ...
        <div class="mx-8 grid grid-rows-1 grid-flow-col border-[1px] border-black rounded-md">
          <button class="rounded-md" :class="[dataProcess.hop == 1 ? 'active':'']" @click="changeHop(1)">1 HOP</button>
          <button class="rounded-md" :class="[dataProcess.hop == 2 ? 'active':'']" @click="changeHop(2)">2 HOP</button>
          <button class="rounded-md" :class="[dataProcess.hop == 3 ? 'active':'']" @click="changeHop(3)">3 HOP</button>
        </div>
        <div class="flex justify-center my-8">
          <div class="text-center text-[30px] font-fungo-800 text-fungo-darkblue">
            <h2>Distance</h2>
            <div class="md:h-[325px]">
          <VelocityInput label-velocity="" max-velocity="99999" :key="change" v-model="dataProcess.distance"/>
          <div class="grid mt-5">
            <button
