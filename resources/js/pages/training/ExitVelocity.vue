...
const dataProcess = ref({
  trajectory: training.trainingActive.trajectory ?? 'N',
...
const save = async () => {
  ...
  let dataToSave = {
    ...
    trajectory: dataProcess.value.trajectory,
    sort: training.trainingActive.sort,
    set: training.countThrowArray[playerCard.value.id].set
    // set: set.value,
  }
  try {
    if(dataProcess.value.trajectory != null && dataProcess.value.trajectory != ""){
      if(training.trainingActive.set != null){
        await axiosPut('result/exitvelocity/' + training.trainingActive.id, {
      ...
      await axiosPost('result/exitvelocity', dataToSave).then(async (response) => {
      if (response) {
        isLoading.status = !isLoading.status;
...
const goToStadistics = () => {
  // router.push({ name: 'training.statsMode', params: { 'idPractice': dataProcess.value.practice, 'mode': "EV" } })
  let link = router.resolve({ name: 'training.statsMode', params: { 'idPractice': dataProcess.value.practice, 'mode': "EV" } })
...
const getInfoThrows = async ()=>{
    ...
    // })
    isLoading.status = !isLoading.status;
    await axiosGet(`statistics/${training.trainingActive.id}/exitvelocity`).then((response)=> {
      if (response) {
        let infoObject = response.data.data.sets;
...
const changeTrajectory = (value)=> {
  if(dataProcess.value.trajectory == value){
    dataProcess.value.trajectory = ''
  }else{
    dataProcess.value.trajectory = value
  }
}
      ...
      <div class="">
        <div class="flex flex-col md:flex-row gap-4 lg:gap-12 justify-evenly lg:justify-start h-auto md:h-[6em] bg-white rounded rounded-lg">
          <div class="flex">
            ...
            <div class="lg:ml-5">
              <h3 class="text-fungo-red  text-[0.6em] lg:text-[1.2em] fungo-700  mt-4 lg:mt-2">Training mode</h3>
              <h1 class="text-fungo-darkblue text-[1.2em] font-fungo-800 lg:text-[1.7em] lg:-mt-2">Exit Velocity</h1>
            </div>
          </div>
          <div
    ...
    <div class="grid grid-flow-row md:grid-cols-2 xl:grid-cols-3 justify-center mt-2 xl:mt-5 gap-3">
        ...
        <div class="flex justify-center">
          <div class="mt-5 order-1 xl:col-span-5">
            <div class="grid grid-cols-2 bg-white balls-count relative shadow-lg mb-8">
              <div class="flex items-center text-fungo-darkblue font-fungo-800 text-[16px] px-2">
                Set :
              </div>
              <div
            ...
            <div class="text-[16px] xl:text-[24px] text-fungo-blue2 mb-4">
              <button @click="changeSet">new set +</button>
            </div>

            <div v-if="training.trainingActive.set == null" class="text-[16px] xl:text-[24px] text-fungo-blue2 mb-4">
              <button @click="goToStadistics">show statistics ></button>
              <!-- <button @click="router.push({ name: 'training.statsMode', params: { 'idPractice': dataProcess.practice, 'mode': 'EV'} })">show statistics ></button> -->
            </div>

            <div v-if="training.trainingActive.set == null" class="text-[16px] xl:text-[24px] text-fungo-blue2 mb-4">
              <button @click="openModal">end exit velocity practice ></button>
            </div>
            <div v-if="training.trainingActive.set == null">
              <h2 class="text-fungo-darkblue font-fungo-800">Training Statistics</h2>
      ...
      <div class="bg-white p-3 ">
        <div class="grid grid-rows-1 grid-flow-col justify-start">
          <TrainingModeIconVue class="mt-2" color="082247" height="75" width="75"/>
          <h2 class="text-[30px] font-fungo-800 text-fungo-red place-self-center">Exit Velocity</h2>
        </div>
        <h3 class="text-fungo-red mx-8 font-bold">Select</h3>
        <div class="mx-8 grid grid-rows-1 grid-flow-col border-[1px] border-black rounded-md">
          <button class="rounded-md" :class="[dataProcess.trajectory == 'GB' ? 'active':'']" @click="changeTrajectory('GB')">GROUND BALL</button>
          <button class="rounded-md" :class="[dataProcess.trajectory == 'LD' ? 'active':'']" @click="changeTrajectory('LD')">LINE DRIVE</button>
          <button class="rounded-md" :class="[dataProcess.trajectory == 'FB' ? 'active':'']" @click="changeTrajectory('FB')">FLY BALL</button>
        </div>
        <div class="flex justify-center my-8">
          <div class="text-center text-[30px] font-fungo-800 text-fungo-darkblue">
            <h2>Velocity</h2>
...
.button-ct {
  @apply border border-fungo-darkblue rounded h-[2em] w-[5em]  focus:bg-fungo-darkblue focus:text-white;
}
