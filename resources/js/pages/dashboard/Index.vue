...
const percentileLeaderboardSnapshots = ref([])
const percentileLeaderboardLoading = ref(false)
const percentileLeaderboardError = ref('')
const percentileLeaderboardTeamId = ref('')
const selectedPercentileCategory = ref('')
const selectedPercentileMetricKey = ref('')
let percentileLeaderboardRequestId = 0

const percentileLeaderboardRows = computed(() => buildTeamPercentileRows(percentileLeaderboardSnapshots.value))
const percentileLeaderboardMetrics = computed(() => teamPercentileMetricOptions(percentileLeaderboardRows.value))
const percentileLeaderboardCategories = computed(() => [...new Set(percentileLeaderboardMetrics.value.map((metric) => metric.category))])
const percentileMetricsForCategory = computed(() => percentileLeaderboardMetrics.value.filter((metric) => metric.category === selectedPercentileCategory.value))
const selectedPercentileMetric = computed(() => percentileLeaderboardMetrics.value.find((metric) => metric.key === selectedPercentileMetricKey.value) || null)
const rankedPercentilePlayers = computed(() => rankTeamPercentileRows(percentileLeaderboardRows.value, selectedPercentileMetricKey.value, 25))

watch(percentileLeaderboardMetrics, (metrics) => {
  if (!metrics.length) {
    selectedPercentileCategory.value = ''
    selectedPercentileMetricKey.value = ''
    return
  }
  if (!metrics.some((metric) => metric.category === selectedPercentileCategory.value)) {
    selectedPercentileCategory.value = metrics[0].category
  }
  if (!metrics.some((metric) => metric.key === selectedPercentileMetricKey.value && metric.category === selectedPercentileCategory.value)) {
    selectedPercentileMetricKey.value = metrics.find((metric) => metric.category === selectedPercentileCategory.value)?.key || metrics[0].key
  }
})

watch(selectedPercentileCategory, (category) => {
  const options = percentileLeaderboardMetrics.value.filter((metric) => metric.category === category)
  if (!options.some((metric) => metric.key === selectedPercentileMetricKey.value)) {
...
const fetchPercentileLeaderboard = async (force = false) => {
  ...
  if (!teamId) return
  if (!force && percentileLeaderboardTeamId.value === teamId) return

  const requestId = ++percentileLeaderboardRequestId
  percentileLeaderboardLoading.value = true
  percentileLeaderboardError.value = ''
  try {
    const { data, resolvedTeamId } = await withTeamIdFallbackGet((id) => `coach/teams/${id}/intelligence?days=365`)
    if (requestId !== percentileLeaderboardRequestId) return
    const payload = Array.isArray(data?.data?.players) ? data.data : data
    percentileLeaderboardSnapshots.value = Array.isArray(payload?.players) ? payload.players : []
    percentileLeaderboardTeamId.value = String(resolvedTeamId || teamId)
  } catch (error) {
    if (requestId !== percentileLeaderboardRequestId) return
    percentileLeaderboardSnapshots.value = []
    percentileLeaderboardError.value = error?.response?.data?.message || 'Age-adjusted percentile rankings could not be loaded.'
  } finally {
    if (requestId === percentileLeaderboardRequestId) percentileLeaderboardLoading.value = false
  }
}
...
watch(
  ...
  async (teamId) => {
    ...
    leaderboardServer.value = null
    leaderboardError.value = ''
    ++percentileLeaderboardRequestId
    percentileLeaderboardSnapshots.value = []
    percentileLeaderboardLoading.value = false
