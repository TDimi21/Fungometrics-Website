...
const allSessions = fs.readFileSync(path.join(root, 'resources/js/pages/training/AllSessions.vue'), 'utf8')
const playerConstants = fs.readFileSync(path.join(root, 'resources/js/features/player-home/lib/constants.js'), 'utf8')
...
describe('web and app session report parity', () => {
  ...
  it('uses one canonical row extractor for every API-backed report', () => {
    for (const type of ['batting', 'bullpen', 'cage', 'exit_velocity', 'long_toss', 'weight_ball', 'live_ab']) {
      expect(report).toContain(`extractSessionRows(rd, '${type}')`)
    }
  })

  it('routes Live AB sessions to the unified report from coach and player lists', () => {
    expect(report).toContain("live_ab:      { label: 'LIVE AB SESSION REPORT'")
    expect(allSessions).toContain("live: 'live_ab'")
    expect(playerConstants).toContain("L: 'live_ab'")
  })
})
