...
use App\Http\Requests\Api\Training\Result\BattingPracticeEditRequest;
...
class BattingEditResultPracticeTest extends TestCase
{
    ...
    public function test_batting_practice_result_with_hit_ok(): void
    {
        ...
        $practice = BattingPracticeResult::factory()->create();
        $data = [
            'is_contact' => true,
        ...
        $response = $this->json('PUT', "api/result/batting/{$practice->id}", $data);
    ...
    {
        $data = [];
        $response = $this->json('PUT', "api/result/batting/{fake()->uuid()}", $data);
    ...
    {
        $this->mock(BattingPracticeEditRequest::class, function ($mock): void {
